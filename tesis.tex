...
\end{definition}
\textcolor{blue}{Ejemplo de sistema semi: flujo de una ecuación con retardo}
...
\begin{definition}
    Definimos a la órbita de $x$ como $\gamma(x) \coloneqq \Phi(I_x \times \{x\}) \subseteq M$, a la forward orbit como $\gamma_+(x) \coloneqq \Phi((0,T_+(x)),x)$ y a la backward orbit como $\gamma_-(x) \coloneqq \Phi((0,T_-(x)),x)$.
\end{definition}
Es claro que $\gamma(x) = \gamma_\_(x) \cup \{x\} \cup \gamma_+(x)$.
\begin{proposition}    
Si $y \in \gamma(x)$, entonces $\gamma(x) = \gamma(y)$.
\end{proposition}
\begin{proof}
    Supongamos $y \in \gamma(x)$, es decir, $y = \Phi(t,x)$ para algún $t$.
    \\
    Queremos ver que $\gamma(x) \subseteq \gamma(y)$ y que $\gamma(y) \subseteq \gamma(x)$.
    \\
    \\
    Dado $z \in \gamma(x)$, existe algún $s$ tal que $z = \Phi(s,x)$.
    \\
    Por las propiedades del flujo, vale que
    \[
    z \quad = \Phi(s,x) = \Phi(s-t+t, x) = \Phi(s-t, \Phi(t,x)) = \Phi(s-t, y) \quad \in \gamma(y)
    \]
    Análogamente, dado $w \in \gamma(y)$, existe algún $u$ tal que $w = \Phi(u,y)$.
    \\
    Y vale que
    \[
    w \quad = \Phi(u,y) = \Phi(u, \Phi(t,x)) = \Phi(u+t,x) \quad \in \gamma(x)
    \]
\end{proof}
Definimos en $M$ la relación dada por coincidencia de órbitas, es decir, $x \simeq_M y$ si y sólo si $\gamma(x) = \gamma(y)$.
\begin{definition}
    Decimos que $x \in M$ es un punto fijo de $\Phi$ si $\gamma(x) = \{x\}$, y que es regular si no.
\end{definition}
\begin{definition}
...
\\
\\
En particular, una órbita es \textit{periódica} si uno (y, en consecuencia, todos) de sus puntos es periódico, y no es difícil ver que $x$ es periódico si y sólo si $\gamma_+(x) \, \cap \, \gamma_-(x) \neq \emptyset$.
\\
\\
...
\begin{definition}
    Un conjunto $U \subseteq M$ es $\sigma$\textit{-invariante} si para todo  $x \in U$ vale que $\gamma_\sigma(x) \subseteq U$. Es \textit{invariante} si es $\pm$-invariante, es decir, para todo $x \in U$ es $\gamma(x) \in U$.
\end{definition}
\begin{definition}
...
\begin{proposition}
     Si $y \in \gamma(x)$, $\omega_{\pm}(x) = \omega_{\pm}(y)$.
\end{proposition}
\begin{proof}
    Sea $y \in \gamma(x)$. Queremos ver que $\omega_{\pm}(x) = \omega_{\pm}(y)$, es decir, que $\omega_{\pm}(y) \subseteq \omega_{\pm}(x)$ y que $\omega_{\pm}(x) \subseteq \omega_{\pm}(y)$.
    \\
    \\
    Consideremos entonces $z \in \omega_{\pm}(y)$ y veamos que $z \in \omega_{\pm}(x)$. Como $y \in \gamma(x)$, vale que $\Phi(t,x) = y$ para algún $t$. Como $z \in \omega_{\pm}(y)$, hay una sucesión $t_n \rightarrow \pm \infty$ tal que $\Phi(t_n, y) \rightarrow z$.
    \\
    \\
    ...
    \\
    \\
    Que $\omega_{\pm}(x) \subseteq \omega_{\pm}(y)$ es análogo, pues $y \in \gamma(x)$ implica que $x \in \gamma(y)$.
\end{proof}
Dado un conjunto $X \subseteq M$, es claro que el conjunto 
\[
\gamma_{+}(X) = \displaystyle \bigcup_{t>0} \Phi(t,X) = \bigcup_{x \in X} \gamma_+(x)
\]
es un conjunto $+$-invariante por definición. En particular, la clausura $\overline{\gamma_{+}(X)}$ es un conjunto cerrado $+$-invariante. Lo mismo vale para el análogo $\gamma_-(X)$.
\\
\\
...
\begin{proof}
    ...
    \\
    Por la transitividad topológica de $\Lambda$, dado $U \subseteq L$ abierto existe un $t$ tal que $\Phi(t, U) \cap V \neq \emptyset$. Es decir, existen $u \in U$ y $v \in V$ tales que $\Phi(t,u) = v$, y por lo tanto $v \in \gamma(u)$. Como $L$ es invariante, $\gamma(u) \subseteq L$, lo que implica que $v \in L$.
    \\
    \\
...
\end{proposition}
\begin{proof}
    $\Leftarrow)$ Sea $\gamma(x) \subseteq \Lambda$ una órbita densa.
    \\
    Queremos ver que, dados $U, V \subseteq \Lambda$ abiertos, existe un $t$ tal que $\Phi(t,U) \cap V \neq \emptyset$.
    \\
    \\
    Como $U, V$ son abiertos y $\gamma(x)$ densa, vale que $\gamma(x) \cap U \neq \emptyset$ y que $\gamma(x) \cap V \neq \emptyset$.
    \\
    ...
    \\
    Queremos ver que el conjunto $\{x \in \Lambda: \gamma(x) \text{ es densa}\}$ es no vacío.
    \\
    \\
    Como estamos en $\mathbb{R}^n$, existe $\{L_n\}_n$ una base numerable de abiertos de $\Lambda$. Para cada $n$, definimos $A_n = \{x \in \Lambda : \gamma(x) \, \cap \, L_n \neq \emptyset\}$.
    \\
    \\
    Si vemos que $A \coloneqq \displaystyle \bigcap_n A_n$ es no vacía, entonces existe $x_0 \in \Lambda$ tal que para todo $n$ es $\gamma(x_0) \cap L_n \neq \emptyset$. Como $\{L_n\}_n$ es una base de abiertos, $\gamma(x_0)$ es densa.
    \\
    ...
    \\
    Veamos que son densos: dados $U \subseteq \Lambda$ abierto y $n \in \mathbb{N}$, queremos ver que $U \cap A_n$ es no vacía, es decir, que existe $x \in U$ tal que $\gamma(x) \cap L_n$ es no vacía. En efecto, como $\Lambda$ es topológicamente transitivo, existe un $t$ tal que $\Phi(t,U) \cap L_n \neq \emptyset$, es decir, un $x \in U$ tal que $\Phi(t, x) \cap L_n \neq \emptyset$, que era lo que queríamos.
\end{proof}
...
\end{proof}
Queremos probar que caos en el sentido de Devaney's implica caos en el sentido de Li-Yorke. Para eso, primero enunciamos los siguientes resultados:
\begin{corollary}
...
\\
\\
Recordemos que, si $x \in \textit{Tran}_f$, vale que $\omega(x,f) = X$. Dado $x \in \textit{Tran}_f$, para cada $0 \leq i \leq n-1$ definimos $D_i = \omega(f^i(x), f^n) = \{z \in X : f^{nk}(f^i(x)) \xrightarrow{k} z\}$, el conjunto límite del punto $f
^i(x)$ bajo $f^n$.
\\
...
\end{definition}
Sean $X = \mathbb{D}^n$, $A = \partial \mathbb{D}^n = \mathbb{S}^{n-1}$. Sea $f: \partial \mathbb{D}^n \rightarrow Y$ una función continua. Decimos que el espacio de adjunción $Y \cup_f \mathbb{D}^n$ se obtiene de $Y$ \textit{adjuntándole una $n$-celda}, y lo notamos $Y \cup e^n$:
\[
\begin{tikzcd}
...
\begin{definition}
    Dado $r \geq 0$ y $A$ un subconjunto de un espacio métrico $(X,d_X)$, definimos el r-thickening de $A$ como $Or(A) = \{ x \in X : d_X(x,A) < r \}$.
\end{definition}
\begin{definition}
    Dado $X \subseteq \mathbb{R}^d$, definimos su espacio tangente $T(X) = \{(x,v) \in X \times \mathbb{R}^d : v \text{ es el vector tangente de } x\}$. \textcolor{blue}{Ah, acá estaba igual :)}
\end{definition}
...
\\
\\
\subsubsection*{La importancia del radio de thickening y el espacio tangente, descripción gráfica}
...
\end{tikzcd}
\]
es conmutativo. \cite{cycling-sigs} Y por lo tanto nos basta con entender cómo calcular la base del morfismo $\operatorname{im}H_1(\phi)$, que es lo que veremos en el siguiente capítulo.
\newpage
...
\\
\\
Pero antes, veamos la importancia del espacio de comparación usando como ejemplo el atractor de Double-Well.
